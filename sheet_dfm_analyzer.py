...
class SheetMetalAnalyzer:
    ...
    def __init__(self, step_file: str, config: dict):
    ...
    def check_large_part_for_material(self, solids: list) -> dict:
        ...
        cfg['sheet_max_part_volume_cm3'].  Sheet metal parts are typically flat and
        ...
        try:
            ...
            limit = self.cfg["sheet_max_part_volume_cm3"]
    ...
    def check_small_holes(self, solids: list) -> dict:
        """
        Detect holes whose diameter is below cfg['sheet_min_hole_diameter_mm'].
        ...
        name = "Small Holes"
        try:
            min_dia = self.cfg["sheet_min_hole_diameter_mm"]
            all_holes: list[dict] = []
            issues: list[dict] = []
    ...
    def _check_solid_fidelity(self, cq_shape: cq.Shape, cfg: dict) -> dict:
        ...
        try:
            ...
            min_faces = cfg.get("sheet_fidelity_min_faces")
    ...
    def check_thickness(self, solids: list) -> dict:
        """
        Verify detected sheet thickness for each solid individually
        against configured min/max bounds.
        """
        name = "Thickness Check"
        try:
            min_t = self.cfg.get("sheet_min_materail_thickness_mm")
            max_t = self.cfg.get("sheet_max_materail_thickness_mm")
    ...
    def check_bends(self, solids: list) -> dict:
        """
        Verify bend count does not exceed cfg['max_bends'].
        """
        name = "Bend Count Check"
        try:
            max_bends = self.cfg.get("sheet_max_bends")
            total_bends = 0
            ...
            for solid in solids:
                ...
                thickness = self.detect_thickness(planar_faces, closed_axis_groups)
                total_bends += self.detect_bends(partial_axis_groups, thickness)

            if total_bends > max_bends:
                return {
                    "check": name,
                    "status": "FAIL",
                    "message": f"Bend count {total_bends} exceeds maximum allowed {max_bends}.",
                    "details": {
                        "detected_bends": total_bends,
                        "max_bends": max_bends,
                        "advice": "Simplify the part or split into multiple components.",
                    },
                }
            ...
            return {
                "check": name,
                "status": "PASS",
                "message": f"Bend count {total_bends} is within the allowed maximum ({max_bends}).",
                "details": {"detected_bends": total_bends, "max_bends": max_bends},
            }
    ...
    def check_non_sheet_features(self, solids: list) -> dict:
        """
        Flag non-sheet-metal features (spheres, tori, etc.) unless
        cfg['allow_non_sheet_features'] is True.
        """
        name = "Non-Sheet Features"
        try:
            if self.cfg.get("sheet_allow_non_sheet_features"):
                return {
                    "check": name,
                    "status": "SKIP",
                    "message": "Non-sheet feature check disabled by configuration.",
                    "details": {},
                }

            flagged_solids = []
            for idx, solid in enumerate(solids):
                if self.detect_non_sheet_features(solid.Faces()):
                    flagged_solids.append(idx)
            ...
            if flagged_solids:
                return {
                    ...
                    "status": "FAIL",
                    "message": (
                        f"Non-sheet features (sphere/torus) detected in "
                        f"{len(flagged_solids)} solid(s): {flagged_solids}."
                    ),
                    "details": {
                        "affected_solids": flagged_solids,
                        "advice": (
                            "Remove or replace non-planar/non-cylindrical features "
                            "incompatible with sheet metal fabrication."
                        ),
                    },
                }
            ...
            return {
                "check": name,
                "status": "PASS",
                "message": "No non-sheet features detected.",
                "details": {},
            }
    ...
    def analyze(self):
        ...
        dfm_checks = [
            ...
            self.check_bends(solids),
            self.check_non_sheet_features(solids),
